...
ll getHash(int l, int r, int id) {
    return sub(h[r][id], mul(h[l - 1][id], p[r - l + 1]));
}
...
void Solve() {
    ...
    for (int i = 1; i <= n; i++) {
        h[i][1] = add(mul(h[i - 1][1], BASE), s[i] - 'A' + 1);
    }
    for (int i = 1; i <= m; i++) {
        h[i][2] = add(mul(h[i - 1][2], BASE), t[i] - 'A' + 1);
    }
    ans = n + 1;
    for (int i = n; i; i--) {
        ans = (getHash(i, n, 1) == getHash(1, n - i + 1, 2) ? i : ans);
    }
