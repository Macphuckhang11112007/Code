...
ll mul(ll x, ll y) {
    return lll(1) * x * y % MOD;
}

ll getHash(int l, int r) {
    return (h[r] - mul(h[l - 1], p[r - l + 1]) + MOD) % MOD;
}

ll getRHash(int l, int r) {
    return (rh[l] - mul(rh[r + 1], p[r - l + 1]) + MOD) % MOD;
}
...
void Solve() {
    ...
    for (int i = n; i; i--) {
        (rh[i] = mul(rh[i + 1], BASE) + s[i] - 'A' + 1) %= MOD;
    }
    while (q--) {
        int l, r;
        cin >> l >> r;
        int d = (r - l + 1) / 2;
        cout << (getHash(l, l + d - 1) == getRHash(r - d + 1, r) ? "YES" : "NO") << "\n";
    }
    return;
}
